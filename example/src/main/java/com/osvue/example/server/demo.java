...
public class demo {
  ...
  private static void printProcessor(CentralProcessor processor) {
    ...
    System.out.println(" " + processor.getPhysicalProcessorCount() + " physical CPU package(s)");
  ...
  private static void printFileSystem(FileSystem fileSystem) {
    System.out.println("File System:");
      ...
      System.out.format(" File Descriptors: %d/%d%n", ((oshi.software.os.FileSystem)
      fileSystem).getOpenFileDescriptors(), ((oshi.software.os.FileSystem)
      fileSystem).getMaxFileDescriptors());
      
      OSFileStore[] fsArray = fileSystem.getFileStores(); for (OSFileStore fs : fsArray) { long
      usable = fs.getUsableSpace(); long total = fs.getTotalSpace(); System.out.format(
      " %s (%s) [%s] %s of %s free (%.1f%%) is %s " + (fs.getLogicalVolume() != null &&
      fs.getLogicalVolume().length() > 0 ? "[%s]" : "%s") + " and is mounted at %s%n",
