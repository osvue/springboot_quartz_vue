...
import com.osvue.example.utils.eladmin.utils.FileUtil;
...
//    OSFileStore[] fsArray = fileSystem.getFileStores();
  ...
  private Map<String, Object> getSwapInfo(GlobalMemory memory) {
    ...
    swapInfo.put("available", FormatUtil.formatBytes(
        memory.getVirtualMemory().getSwapTotal() - memory.getVirtualMemory().getSwapUsed()));
    swapInfo.put("usageRate", df.format(memory.getVirtualMemory().getSwapUsed()
        / (double) memory.getVirtualMemory().getSwapTotal() * 100));
    return swapInfo;
  }
